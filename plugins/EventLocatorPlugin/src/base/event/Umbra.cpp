//$Id: Umbra.cpp 2124 2012-02-21 19:24:34Z djconway@NDC $
//------------------------------------------------------------------------------
//                           Umbra
//------------------------------------------------------------------------------
...
//
/**
 * Implementation of the Umbra EventFunction
 */
//------------------------------------------------------------------------------
 ...
 */
//------------------------------------------------------------------------------
Umbra& Umbra::operator=(const Umbra& u)
{
   if (this != &u)
   {
      Eclipse::operator=(u);
 ...
 */
//------------------------------------------------------------------------------
Real* Umbra::Evaluate(GmatEpoch atEpoch, Real* forState)
{
   #ifdef DEBUG_EVENTFUNCTIONS
      MessageInterface::ShowMessage("Evaluating Umbra\n   primary <%p>, body<%p>, "
            "sol <%p>\n", primary, body, sol);
   #endif
   ...
   if ((primary != NULL) && (body != NULL) && (sol != NULL))
   {
      ...
      {
         if (atEpoch < 0.0)
            throw EventException("Epoch passed into an Umbra event is invalid.");
         if (forState == NULL)
            throw EventException("State passed into an Umbra event is NULL.");
         now = atEpoch;
      ...
      Real pi, theta, alpha, eta; //, gamma;
      pi = GmatMathConstants::PI;

      #ifdef DEBUG_EVENTFUNCTIONS
         MessageInterface::ShowMessage("Umbra Calcs:\n   starRadius = %.12le, "
   ...
   else
   {
      ...
      throw EventException("Error calculating Umbra; object undefined." +
            details);
   }
