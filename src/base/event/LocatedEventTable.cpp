...
//   Subscriber              (let),
   events                  (let.events),
   requiredEventType       (let.requiredEventType)
...
//   if (active)
//   {
//   if (theEvent->IsOfType(requiredEventType))
      events.push_back(theEvent);
   // do we need an exception here?  I don't think so - that would be a developer error
