...
Integrator::Integrator(const std::string &typeStr, const std::string &nomme)
    : Propagator              (typeStr, nomme),
      ...
      fixedStep               (false),
      fixedStepsize           (300.0),
      minimumStep             (1.0e-7),				//(0.001),					// Requirement for minimumStep is that: minimumStep < 1/2 x smallestTime	// made changes by TUAN NGUYEN
      maximumStep             (2700.0),               // 45 minutes				// Requirement for maximumStep is that: maximumStep > 2 x smallestTime		// made changes by TUAN NGUYEN
      smallestTime            (1.0e-6),
      stepAttempts            (0),
      maxStepAttempts         (50),
