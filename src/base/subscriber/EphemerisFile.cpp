...
bool EphemerisFile::IsTimeToWrite(Real epochInSecs, const Real state[6])
{
   ...
   if (useStepSize)
   {
      ...
      if (writingNewSegment)
      {
         ...
         // If user initial epoch is defined, set it to nextOutEpoch
         if (initialEpochA1Mjd != -999.999)
            nextOutEpoch = initialEpochA1Mjd * GmatTimeConstants::SECS_PER_DAY;
      ...
      else
      {
         ...
         if (handleFinalEpoch && epochInSecs >= nextOutEpoch)
         {
...
void EphemerisFile::WriteOrbit(Real reqEpochInSecs, const Real state[6])
{
   #ifdef DEBUG_EPHEMFILE_WRITE
   MessageInterface::ShowMessage
      ("EphemerisFile::WriteOrbit() entered, processData=%d\n", processData);
...
bool EphemerisFile::Distribute(const Real * dat, Integer len)
{
   ...
   if (isEndOfReceive && isEndOfDataBlock)
   {
      ...
      if (!isEndOfRun)
      {
         std::string comment = "This block begins after ";
         if (runstate == Gmat::SOLVEDPASS)
            comment = "This block begins after final target sequence at ";
         
         // Convert last epoch in secs to gregorian format
         comment = comment + lastEpochWroteStr;
         ...
         #ifdef DEBUG_EPHEMFILE_FINISH
         MessageInterface::ShowMessage
            ("   lastCommentBegPos=%ld saved\n", (long)lastCommentBegPos);
         #endif
      ...
      {
         // End of run, blank out the last comment
         #ifdef DEBUG_EPHEMFILE_FINISH
         MessageInterface::ShowMessage("=====> End of run, Removing the last comment\n");
         #endif
         std::string comment = "                                                  ";
   ...
   if (runstate == Gmat::RUNNING || runstate == Gmat::SOLVEDPASS)
   {
      ...
      MessageInterface::ShowMessage
         ("   Start writing data, currEpochInDays=%.15f, currEpochInSecs=%.15f, %s\n"
          "   writeOrbit=%d, writeAttitude=%d\n", currEpochInDays, currEpochInSecs,
      ...
      if (fileType == SPK_ORBIT)
         ...
         HandleCcsdsOrbitData(processData);
   }
                                      ...
                                      const std::string &desc)
{
   ...
   // If runstate changed, write any remaining data in the buffer
   //FinishUpWriting();
