 ...
 */
//------------------------------------------------------------------------------
std::string BranchCommand::GetCurrentGeneratingString(Gmat::WriteMode mode, const std::string &prefix)
{
   ...
   if ((current != this) && (current != NULL))
   {
      ...
      if (current->IsOfType("BranchCommand"))
      {
         std::string substring = (((BranchCommand*)current)->GetCurrentGeneratingString(mode, myPrefix + "--> "));
         if (substring != "")
            theString = substring;
      }
   }
