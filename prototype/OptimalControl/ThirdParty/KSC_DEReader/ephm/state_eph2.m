function [pv,vv] = state_eph2(jd,list,baryflag,kmflag,Sec,SecFraction)
...
% if abs(jd - floor(jd) - DayFraction) > 0.001
%     disp('Some kind of error in state_eph1');
%     pause
% end
...
% db(1)
% ts
ref_sec = (db(1)-2430000) * 86400 ;

delta_sec = Sec - ref_sec;

% delta_sec/86400


temp1 = delta_sec / (ts * 86400);
temp2 = SecFraction / (ts * 86400);




% t_temp1 = (floor(jd) - db(1))/ts;
% t_temp2 = DayFraction/ts;
...
pv=zeros(3,12);
vv=zeros(3,12);
au2km=AU; % define au2km conversion 
