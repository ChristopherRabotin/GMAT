    ...
    elseif strcmp(s(1).type,'.')         % index reference a.x or a.dx
      ...
      elseif strcmp(s(1).subs,'dx')
        ...
        if issparse(a.dx) & ( length(sizeadx)>1 )
          ...
          if INTLAB_GRADIENT_DERIV_ERROR==1
            error('access of .dx of higher dimensional sparse gradient, see gradientinit')
          elseif INTLAB_GRADIENT_DERIV_ERROR==0
            warning('gradient is stored 2-dimensional because Matlab does not allow multi-dimensional sparse arrays')
          end
        end
