...
global INTLAB_INTVAL_ETA
  ...
  x = ( x + (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_1 ) + ... % exact for abs(x)<=2^27
        (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_2;

  setround(-1)
  zinf = x + (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_3SUP;
  zinf1 = ( x + (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_3 ) + ...
            (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_4SUP;
  zinf = max( zinf , zinf1 );

  setround(1)
  zsup = x + (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_3INF;
  zsup1 = ( x + (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_3 ) + ...
            (-k)*INTLAB_INTVAL_STDFCTS_PI.PI2_4INF;
  zsup = min( zsup , zsup1 );

  zinf1 = zinf + INTLAB_INTVAL_ETA;  % rounding upwards
  ...
  indexpoor = indexpoor | ( zinf>INTLAB_INTVAL_STDFCTS_PI.PI4INF );
...
global INTLAB_INTVAL_STDFCTS_PI
  ...
  kmax = 6;
  if n==0
    y = conv( INTLAB_INTVAL_STDFCTS_PI.INV_PI2(3+q-2:3+q+kmax) , F );
  else
    y = zeros(n,kmax+5);
    Ones = ones(1,kmax+3);
    v = 3 + q*Ones + ones(n,1)*(-2:kmax);
    y(:,1:kmax+3) = (F(:,1)*Ones) .* INTLAB_INTVAL_STDFCTS_PI.INV_PI2(v);
    y(:,2:kmax+4) = y(:,2:kmax+4) + ...
       (F(:,2)*Ones) .* INTLAB_INTVAL_STDFCTS_PI.INV_PI2(v);
    y(:,3:kmax+5) = y(:,3:kmax+5) + ...
       (F(:,3)*Ones) .* INTLAB_INTVAL_STDFCTS_PI.INV_PI2(v);
  end
  ...
  % multiply by pi/2
  z = conv2( y , INTLAB_INTVAL_STDFCTS_PI.PI2_4 );
