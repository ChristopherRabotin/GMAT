...
%AAAA Matlab V5.2 bug fix
  ...
  global INTLAB_DISPLAY_WIDTH    % minimum 110, so columns>=1
  ...
  if nargout & isempty(name) & isempty(restricted)
    ...
    if c.complex           % complex intervals
      ...
      for i=1:m
        s = '';
        strrealmid = sprintf(formatstr,real(cmid(i))/factor);
        signimagmid = sign(imag(cmid(i)));
        strimagmid = sprintf(formatstr,abs(imag(cmid(i)))/factor);
        ...
        setround(1)
        re = max( real(cmid(i))-cmidrealinf,cmidrealsup-real(cmid(i)) );
        im = max( imag(cmid(i))-cmidimaginf,cmidimagsup-imag(cmid(i)) );
        if isequal(crad,0)
      ...
      for i=1:m
        s = '';
        strmid = sprintf(formatstr,cmid(i)/factor);
        [cmidinf,cmidsup] = str2intval(strmid,commonexp);
        setround(1)
        cradij = crad(i) + max(cmid(i)-cmidinf,cmidsup-cmid(i));
        s = [ s '<' strmid ',' ...
  ...
  if nargout
    ...
    if c.complex
      for i=1:prod(size(cmid))
        strrealmid = sprintf(formatstr,real(cmid(i)));
        strimagmid = sprintf(formatstr,imag(cmid(i)));
      ...
      for i=1:prod(size(cmid))
        strmid = sprintf(formatstr,cmid(i));
        [cmidinf,cmidsup] = str2intval(strmid,0);
        setround(1)
        cradi = crad(i) + max(cmid(i)-cmidinf,cmidsup-cmid(i));
  ...
  if issparse(c)
    ...
    if c.complex
      ...
      for i=1:length(I)
        ...
        [cmidinf,cmidsup] = str2intval(strmid,commonexp);
        setround(1)
        cradij = crad(I(i),J(i)) + max(cmidij-cmidinf,cmidsup-cmidij);
        str = [ str '<' strmid ',' ...
                dble2str_rnd(cradij,commonexp,len-1,prec,expon,+1) '> ' ];
        disp( str )
      end
  ...
  for jj=1:ceil(n/columns)
    ...
    if c.complex           % complex intervals
      for i=1:m
        s = '';
        for j = j1:j2
          strrealmid = sprintf(formatstr,real(cmid(i,j))/factor);
          signimagmid = sign(imag(cmid(i,j)));
          strimagmid = sprintf(formatstr,abs(imag(cmid(i,j)))/factor);
          strimagmid = strimagmid(2:end);     % omit leading space
          [cmidrealinf,cmidrealsup] = str2intval(strrealmid,commonexp);
          if signimagmid==1
            [cmidimaginf,cmidimagsup] = str2intval(strimagmid,commonexp);
          ...
          setround(1)
          re = max( real(cmid(i,j))-cmidrealinf,cmidrealsup-real(cmid(i,j)) );
          im = max( imag(cmid(i,j))-cmidimaginf,cmidimagsup-imag(cmid(i,j)) );
          if isequal(crad,0)
            cradij = abs( re + sqrt(-1)*im );
          else
      ...
      for i=1:m
        s = '';
        for j = j1:j2
          strmid = sprintf(formatstr,cmid(i,j)/factor);
          [cmidinf,cmidsup] = str2intval(strmid,commonexp);
          setround(1)
          cradij = crad(i,j) + max(cmid(i,j)-cmidinf,cmidsup-cmid(i,j));
          s = [ s '<' strmid ',' ...
                dble2str_rnd(cradij,commonexp,len-1,prec,expon,+1) '> ' ];
        end
        disp(s)
