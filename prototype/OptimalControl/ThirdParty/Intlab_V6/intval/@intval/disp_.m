...
%
  ...
  global INTLAB_INTVAL_DISPLAY         % Check default display
  ...
  if nargout & isempty(name) & isempty(restricted)
    ...
    if c.complex           % complex intervals
      ...
      for i=1:m
        ...
        strimag = intval2str(abs(imag(cmid(i))),cradij, ...
                             commonexp,len,prec,expon,0);
        str = intval2str(real(cmid(i)),cradij,commonexp,len,prec,expon,0);
      ...
      for i=1:m
        s = intval2str(cmid(i),crad(i),commonexp,len,prec,expon,0);
        outstr.str(i,:) = s;
      end
    end
  ...
  if nargout
    ...
    if c.complex
      for i=1:prod(size(cmid))
        ...
        outstr = [ outstr ...
                   intval2str(real(cmid(i)),cradi,commonexp,len,prec,expon,1) ];
        strim = intval2str(imag(cmid(i)),cradi,commonexp,len,prec,expon,1);
      ...
      for i=1:prod(size(cmid))
        outstr = [ outstr ...
                   intval2str(cmid(i),crad(i),commonexp,len,prec,expon,1) ];
      end
    end
    setround(rndold)
    return
  ...
  if issparse(c)
    ...
    if c.complex
      ...
      for i=1:length(I)
        ...
        str = [ str blanks(20-length(str)) ];
        disp([str ...
        intval2str(cmid(I(i),J(i)),crad(I(i),J(i)),commonexp,len,prec,expon,0) ])
      end
    end
    setround(rndold)
    return
    ...
    if c.complex           % complex intervals
      for i=1:m
        ...
        for j = j1:j2
          ...
          strimag = intval2str(abs(imag(cmid(i,j))),cradij, ...
                               commonexp,len,prec,expon,0);
          str = intval2str(real(cmid(i,j)),cradij,commonexp,len,prec,expon,0);
          s = [ s str ' ' chsign strimag(2:end) 'i ' ];
        end
        disp(s)
      end
      ...
      for i=1:m
        s = '';
        for j = j1:j2
          str = intval2str(cmid(i,j),crad(i,j),commonexp,len,prec,expon,0);
          s = [ s str ];
        end
        disp(s)
      end
