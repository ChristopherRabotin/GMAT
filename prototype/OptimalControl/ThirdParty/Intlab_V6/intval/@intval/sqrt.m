...
%   y.rad = abs( sqrt(abs(x.mid)-x.rad) - sqrt(abs(x.mid)) );
    ...
    index = ( xmidre==0 );
    if any(index(:))
      Phi.inf(index) = INTLAB_INTVAL_STDFCTS_PI.PI2INF;          % pi/2
      Phi.sup(index) = INTLAB_INTVAL_STDFCTS_PI.PI2SUP;
      indexneg = index & ( xmidim<0 );
      if any(indexneg(:))
        Phi.inf(indexneg) = -INTLAB_INTVAL_STDFCTS_PI.PI2SUP;    % -pi/2
        Phi.sup(indexneg) = -INTLAB_INTVAL_STDFCTS_PI.PI2INF;
      end
    end
    ...
    if any(index(:))
      Phi.inf(index) = 0;
      Phi.sup(index) = 2*INTLAB_INTVAL_STDFCTS_PI.PISUP;
    end
  ...
  if any(index(:))                      % handle input out-of-range
    if INTLAB_INTVAL_STDFCTS_EXCPTN<=1  % out-of-range input handled as complex
    ...
    if INTLAB_INTVAL_STDFCTS_EXCPTN==3    % ignore input out of range (ignore-mode)
