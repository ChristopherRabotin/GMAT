...
global INTLAB_INTVAL_STDFCTS_PI
  ...
  if any(index)
    ...
    switch s
      case -1                          % asin(x) - pi/2
        if rnd==-1
          y(index) = atan_pos(xrnd,rnd) - INTLAB_INTVAL_STDFCTS_PI.PI2SUP;
        else
          y(index) = atan_pos(xrnd,rnd) - INTLAB_INTVAL_STDFCTS_PI.PI2INF;
        end
      case 0                           % asin(x)
  ...
  if any(index)                        % .75 <= x <= 1
    ...
    switch s
      ...
      case 0                           % asin(x)
        setround(rnd)
        if rnd==-1
          y(index) = INTLAB_INTVAL_STDFCTS_PI.PI2INF - yrnd;
        else
          y(index) = INTLAB_INTVAL_STDFCTS_PI.PI2SUP - yrnd;
        end
      case 1                           % asin(x) + pi/2
        setround(rnd)
        if rnd==-1
          y(index) = INTLAB_INTVAL_STDFCTS_PI.PIINF - yrnd;
        else
          y(index) = INTLAB_INTVAL_STDFCTS_PI.PISUP - yrnd;
        end
    end
  end
