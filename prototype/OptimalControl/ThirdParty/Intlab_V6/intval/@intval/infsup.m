...
%AAAA Matlab V5.2 bug fix
  ...
  if nargout & isempty(name) & isempty(restricted)
    ...
    if c.complex           % complex intervals
      ...
      for i=1:m
        s = [ '[' ...
              dble2str_rnd(cinf(i),commonexp,len,prec,expon,-1) ',' ...
              dble2str_rnd(csup(i),commonexp,len,prec,expon,+1) '] ' ];
        outstr.str(i,:) = s;
      end
    end
    if rndold
  ...
  if issparse(c)
    ...
    if c.complex
      ...
      for i=1:length(I)
        ...
        str = [ str blanks(20-length(str)) ];
        str = [ str '[' ...
                   dble2str_rnd(cinf(I(i),J(i)),commonexp,len,prec,expon,-1) ',' ...
                   dble2str_rnd(csup(I(i),J(i)),commonexp,len,prec,expon,+1) '] ' ];
        disp( str )
      end
    end
    if rndold
  ...
  for jj=1:ceil(n/columns)
    ...
    if c.complex           % complex intervals
      ...
      for i=1:m
        ...
        for j = j1:j2
          s = [ s '[' ...
                dble2str_rnd(cinf(i,j),commonexp,len,prec,expon,-1) ',' ...
                dble2str_rnd(csup(i,j),commonexp,len,prec,expon,+1) '] ' ];
        end
        disp(s)
      end
    end
