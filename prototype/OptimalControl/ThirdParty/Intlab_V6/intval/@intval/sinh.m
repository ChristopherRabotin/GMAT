...
% rounding corresponding to rnd
  ...
  if any(index)
    ...
    setround(-1)
    expinf = exp(xs)*(1-INTLAB_INTVAL_STDFCTS_EXP.EPS);
    setround(1)
    expsup = exp(xs)*(1+INTLAB_INTVAL_STDFCTS_EXP.EPS);
    expinf(xs==0) = 1;                    % gives accuracy for small x
    expsup(xs==0) = 1;

    % use sinh(xx) = sinh(xs+d) = sinh(xs)*cosh(d) + cosh(xs)*sinh(d)
    setround(rnd)
    corr = INTLAB_INTVAL_STDFCTS_SINH.EPS;
    if rnd==-1
      % coshxs <= cosh(xs)
  ...
  if any(index)
    ...
    setround(rnd)
    if rnd==-1
      y(index) = exprnd .* INTLAB_INTVAL_STDFCTS_E.INF;
    else
      y(index) = exprnd .* INTLAB_INTVAL_STDFCTS_E.SUP;
    end
  end
