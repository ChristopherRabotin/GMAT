...
% modified 05/29/09     S.M. Rump  omit display of INTLAB_PATH 
%
  ...
  global INTLAB_INTVAL_STDFCTS_EPSMAX
  
  ALLDATA = [];                         % struct array for all data
  
  global INTLAB_INTLABPATH
  ALLDATA = storedata(ALLDATA,INTLAB_INTLABPATH,0);
  ...
  INTLAB_INTVAL_STDFCTS_EPSMAX = 1e-15;
  ...
  INTLAB_INTVAL_STDFCTS_SUCCESS = 1;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_SUCCESS);
...
% quit stdfctsdata with no success
  ...
  INTLAB_INTVAL_STDFCTS_SUCCESS = 0;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_SUCCESS);
  
  % make sure all constants not usable
  INTLAB_INTVAL_STDFCTS_LOG.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_LOG);
  INTLAB_INTVAL_STDFCTS_ATAN.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_ATAN);
  INTLAB_INTVAL_STDFCTS_EXP.POW = NaN;
  INTLAB_INTVAL_STDFCTS_EXP.POWINF = NaN;
  INTLAB_INTVAL_STDFCTS_EXP.POWSUP = NaN;
  INTLAB_INTVAL_STDFCTS_EXP.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_EXP);
  INTLAB_INTVAL_STDFCTS_SIN.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_SIN);
  INTLAB_INTVAL_STDFCTS_COS.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_COS);
  INTLAB_INTVAL_STDFCTS_TAN.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_TAN);
  INTLAB_INTVAL_STDFCTS_SINH.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_SINH);
  INTLAB_INTVAL_STDFCTS_TANH.EPS = NaN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_TANH);
...
%
  ...
  for i=0:31          % range ( 1 .. 2^15-1 ) / 2^13
    ...
    disp(['Finished ' int2str(26+(i+1)*1.5)  ' of 100 %'])
    if ~success
      return
    end
  end

  INTLAB_INTVAL_STDFCTS_ATAN = ATAN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_ATAN);
...
%
  ...
  for E=0:1
    for i=0:7
      ...
      disp(['Finished ' int2str(2+8*E+(i+1)) ' of 100 %']);
      if ~success
        return
      end
    end
...
%
  ...
  for i=0:15            % range ( 0 .. 2^14-1 ) / 2^14
    ...
    disp(['Finished ' int2str(74+(i+1)*1.5) ' of 100 %'])
    if ~success
      return
    end
  end

  INTLAB_INTVAL_STDFCTS_EXP = E_EXP;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_EXP);

  INTLAB_INTVAL_STDFCTS_SIN = E_SIN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_SIN);

  INTLAB_INTVAL_STDFCTS_COS = E_COS;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_COS);

  INTLAB_INTVAL_STDFCTS_TAN = E_TAN;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_TAN);

  INTLAB_INTVAL_STDFCTS_SINH = E_SINH;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_SINH);

  INTLAB_INTVAL_STDFCTS_TANH = E_TANH;
  ALLDATA = storedata(ALLDATA,INTLAB_INTVAL_STDFCTS_TANH);
...
global INTLAB_INTVAL_STDFCTS_EPSMAX
  ...
  if any(index)
    ...
    if INTLAB_DATA.EPS > INTLAB_INTVAL_STDFCTS_EPSMAX
      success = 0;
      return
    end
  end
