...
%
%Implements is Algorithm 6.1 in 
...
%
  ...
  x(1:n2) = (2*rand(n2,1)-1).*(2.^e);   % generate first half of vectors
  y(1:n2) = (2*rand(n2,1)-1).*(2.^e);
  
  e = round(linspace(b/2,0,n-n2));      % generate exponents for second half
  for i=n2+1:n
    x(i) = (2*rand-1)*2^e(i-n2);                % generate x(i), y(i) such that x(v)'*y(v)~2^e(i-n2)
    y(i) = ((2*rand-1)*2^e(i-n2)-DotK(x',y,k))/x(i);   % for v=1:i
  end
