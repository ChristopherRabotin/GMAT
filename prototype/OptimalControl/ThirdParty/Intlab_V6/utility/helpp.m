...
%
  ...
  if nargin==0
    ...
    global INTLAB_MATRIX_MEMORY INTLAB_MATRIX_FILES           % matrix memory for INTLAB
    global INTLAB_MATRIX_MEMORY_USER INTLAB_MATRIX_FILES_USER % matrix memory for user-defined path
    global INTLAB_MATRIX_MEMORY_ALL INTLAB_MATRIX_FILES_ALL   % matrix memory for complete path
  ...
  if nargin==1                                  % input only file name, no extra parameter
    global INTLAB_MATRIX_MEMORY INTLAB_MATRIX_FILES
    MEMORY = INTLAB_MATRIX_MEMORY; 
    FILES = INTLAB_MATRIX_FILES;
    level = defaultlevel - max(0,5-length(name));  % level smaller for short names
  else                                          % at least two parameters
    all = lower(all);
    if isequal(all,'all')
      global INTLAB_MATRIX_MEMORY_ALL INTLAB_MATRIX_FILES_ALL
      MEMORY = INTLAB_MATRIX_MEMORY_ALL; 
      FILES = INTLAB_MATRIX_FILES_ALL;
      if nargin~=3
        level = defaultlevel - max(0,5-length(name));  % level smaller for short names
      end
    elseif isequal(all,'user')
      global INTLAB_MATRIX_MEMORY_USER INTLAB_MATRIX_FILES_USER
      MEMORY = INTLAB_MATRIX_MEMORY_USER; 
      FILES = INTLAB_MATRIX_FILES_USER;
      if nargin~=3
