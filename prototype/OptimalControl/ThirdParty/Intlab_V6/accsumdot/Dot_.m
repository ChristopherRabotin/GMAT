...
%   res = dot_(A,x,-1,b,-1);           Inclusion of residual A*x-b in quadruple precision
...
%
    ...
    if ( prod(sA)==1 ) & ( abs(A)==1 )   % first factor A scalar +/- 1
      ...
      if sA(1)~=1                               % m~=1: matrix-vector product
        ...
        if isreal(A) | isreal(x)                % A or x real
          ...
          if issparse(A)
            h = [ sparse(iA,jA,ssA.*real(x(jA)),m,sA(2)) , sparse(iA,jA,I*(ssA.*real(x(jA))),m,sA(2)) ];
          else
            xx = x(:,ones(1,m)).';
  ...
  if abs(K)==2                          % quadruple precision
    if K>0
      ...
      setround(-1)
      resinf = res + ( sinf - 5*idim*INTLAB_INTVAL_ETA );
      setround(1)
      ressup = res + ( ssup + 5*idim*INTLAB_INTVAL_ETA );
    ...
    if K>0
      ...
      setround(-1)
      resinf = res + ( sum(q,2) - 5*idim*INTLAB_INTVAL_ETA );
      setround(1)
      ressup = res + ( sum(q,2) + 5*idim*INTLAB_INTVAL_ETA );
      res = infsup(resinf,ressup);
    end
  end
