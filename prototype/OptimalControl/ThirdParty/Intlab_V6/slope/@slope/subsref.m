    ...
    elseif strcmp(s(1).type,'.')         % index reference a.c, a.r or a.s
      ...
      elseif strcmp(s(1).subs,'s')
        if length(a.size)==2 & ( a.size(2)==1 )
          ...
          if issparse(a.s) & ( prod(a.size)>1 )
            global INTLAB_SLOPE_DERIV_ERROR
            if INTLAB_SLOPE_DERIV_ERROR==1
              error('access of .s of higher dimensional sparse slope, see slopeinit')
            elseif INTLAB_SLOPE_DERIV_ERROR==0
              warning('slope is stored 2-dimensional because Matlab does not allow multi-dimensional sparse arrays')
            end
          end
